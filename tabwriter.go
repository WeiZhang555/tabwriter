...
//
// Modified by WeiZhang555 (zhangwei_cs@qq.com)
package tabwriter
...
const (
	// FilterHTML Ignore html tags and treat entities (starting with '&'
	// and ending in ';') as single characters (width = 1).
	FilterHTML uint = 1 << iota

	// StripEscape Strip Escape characters bracketing escaped text segments
	// instead of passing them through unchanged with the text.
	StripEscape

	// AlignRight Force right-alignment of cell content.
	// Default is left-alignment.
	AlignRight

	// DiscardEmptyColumns Handle empty columns as if they were not present in
	// the input in the first place.
	DiscardEmptyColumns

	// TabIndent marks Always use tabs for indentation columns (i.e., padding of
	// leading empty cells on the left) independent of padchar.
	TabIndent

	// Debug marks print a vertical bar ('|') between columns (after formatting).
	// Discarded columns appear as zero-width columns ("||").
	Debug
)

// Init Writer: A Writer must be initialized with a call to Init. The first parameter (output)
